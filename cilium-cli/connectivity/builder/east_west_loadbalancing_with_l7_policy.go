...
func eastWestLoadbalancingWithL7PolicyTest(ct *check.ConnectivityTest, portRanges bool) {
	...
	// pod-to-remote-nodeport requires KPR when EnableIdentityMark is disabled
	// (e.g., aws-cni), otherwise we cannot derive the security identity of
	// packets arriving from remote nodes via kube-proxy rev-DNAT.
	scenarios := []check.Scenario{tests.PodToLocalNodePort()}
	if ct.Features[features.EnableIdentityMark].Enabled || ct.Features[features.KPR].Enabled {
